...
public static class JsonElementExtensions
{
    ...
    public static List<string> GetUsings(this JsonElement properties, List<ModelNameSpaceInfo> modelNameSpaces)
    {
        ...
        for (int i = 0; i < enumerateProperties.Count; i++)
        {
            ...
            var isPrimitive = TypeResolverUtility.IsPrimitiveType(propType) || TypeResolverUtility.IsNullablePrimitiveType(propType);
            if (isPrimitive)
                continue;

            var nameSpace = TypeResolverUtility.FindModelNameSpace(propType, modelNameSpaces);
        ...
        return [.. result.Distinct().OrderBy(x => x.Length)];
    }
}